...
const updateProfile = async (req, res, next) => {
    try {
        const allowed = ['firstName', 'lastName', 'phone', 'businessName', 'cuisine', 'isOpen', 'deliveryEnabled', 'deliveryFee', 'minOrder', 'openingHours', 'preferences'];
        const updates = {};
...
const changePassword = async (req, res, next) => {
    try {
        ...
        const partner = await RestaurantPartner.findById(req.user._id).select('+password');
...
const forgotPassword = async (req, res, next) => {
    try {
        ...
        const partner = await RestaurantPartner.findOne({ email });
...
const resetPassword = async (req, res, next) => {
    try {
        ...
        const partner = await RestaurantPartner.findOne({ resetPasswordToken: token, resetPasswordExpire: { $gt: Date.now() } });
...
const sendOTP = async (req, res, next) => {
    try {
        ...
        await RestaurantPartner.findByIdAndUpdate(req.user._id, { otpToken: otp, otpExpire: Date.now() + 5 * 60 * 1000 });
...
const verifyOTP = async (req, res, next) => {
    try {
        const { otp } = req.body;
        const partner = await RestaurantPartner.findOne({ _id: req.user._id, otpToken: otp, otpExpire: { $gt: Date.now() } });
