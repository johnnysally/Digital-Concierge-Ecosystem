...
const changePassword = async (req, res, next) => {
    try {
        ...
        const partner = await TransportPartner.findById(req.user._id).select('+password');
...
const forgotPassword = async (req, res, next) => {
    try {
        ...
        const partner = await TransportPartner.findOne({ email });
        ...
        await partner.save();
        const resetLink = `${process.env.PARTNER_URL || 'http://localhost:3000'}/transport-admin/reset-password/${token}`;
...
const resetPassword = async (req, res, next) => {
    try {
        ...
        const partner = await TransportPartner.findOne({ resetPasswordToken: token, resetPasswordExpire: { $gt: Date.now() } });
...
const verifyOTP = async (req, res, next) => {
    try {
        const { otp } = req.body;
        const partner = await TransportPartner.findOne({ _id: req.user._id, otpToken: otp, otpExpire: { $gt: Date.now() } });
