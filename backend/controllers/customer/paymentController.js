...
const createStripePayment = async (req, res, next) => {
    try {
        ...
        const { clientSecret, paymentIntentId } = await stripeService.createPaymentIntent({
            amount,
            currency,
            metadata: { customerId: req.user._id.toString(), bookingId },
        });
...
const confirmStripePayment = async (req, res, next) => {
    try {
        ...
        const payment = await Payment.findOneAndUpdate(
            { transactionId: paymentIntentId },
            { status: status === 'succeeded' ? 'completed' : 'failed', reference: paymentIntentId },
            { new: true }
        );
        ...
        if (status === 'succeeded') {
            await Wallet.findOneAndUpdate(
                { customer: req.user._id },
                { $push: { transactions: { type: 'debit', amount: payment.amount, description: 'Payment', reference: paymentIntentId } } },
                { upsert: true }
            );
        }
...
const mpesaCallback = async (req, res, next) => {
    try {
        ...
        await Payment.findOneAndUpdate({ transactionId: CheckoutRequestID }, { status });
...
module.exports = { createStripePayment, confirmStripePayment, initiateMpesaPayment, mpesaCallback, getPaymentHistory };