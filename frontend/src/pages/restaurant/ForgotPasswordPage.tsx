...
import { forgotPassword } from '../../api/restaurant/authApi';
        ...
        <div className="min-h-screen bg-slate-950 px-4 py-10 text-slate-100 sm:px-6 lg:px-8 flex items-center justify-center">
            <div className="w-full max-w-md rounded-3xl border border-slate-800 bg-slate-900/95 p-10 shadow-2xl">
                <div className="text-center mb-8">
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 text-white font-bold mx-auto">R</div>
                ...
                {submitted ? (
                    <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/30 p-6 text-center">
                        ...
                        <Link to="/restaurant-admin/login" className="mt-4 inline-block text-sm text-sky-400 hover:text-sky-300">← Back to Login</Link>
                    </div>
                ) : (
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <label className="block text-sm text-slate-300">
                            Email address
                            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="restaurant@business.com"
                                className="mt-2 w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white outline-none focus:border-amber-500" />
                        </label>
                ...
                <div className="mt-6 text-center text-sm text-slate-500">
                    <Link to="/restaurant-admin/login" className="font-semibold text-amber-400 hover:text-amber-300">← Sign in</Link>
                </div>
            </div>
        </div>
    );
