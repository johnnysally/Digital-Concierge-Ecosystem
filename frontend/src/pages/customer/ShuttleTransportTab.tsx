...
const ShuttleTransportTab = () => {
    ...
    }, [pickup, dropoff, vehicleType, pickupCoords, dropoffCoords, manualDistance, seats]);

    useEffect(() => {
        if (selectedSeatNumbers.length > seats) setSelectedSeatNumbers(selectedSeatNumbers.slice(0, seats));
    }, [seats]);
    ...
    const handlePayNow = (vehicle: any) => {
        ...
        if (seats > vehicle.availableSeats) { setRideError(`Only ${vehicle.availableSeats} seats available.`); return; }
        if (selectedSeatNumbers.length !== seats) { setRideError(`Please select ${seats} seat(s).`); return; }
        setRideError('');
    ...
    const handlePaymentSuccess = () => {
        ...
        setPickup(''); setDropoff(''); setPickupCoords(''); setDropoffCoords(''); setManualDistance('');
        setSeats(1); setSelectedSeatNumbers([]); setScheduledTime('');
        setFareData(null); setFareError('');
    };

    const getSeatColor = (seatNum: number, totalSeats: number) => {
        if (selectedSeatNumbers.includes(seatNum)) return 'bg-sky-500 text-white border-sky-400';
        if (seatNum > totalSeats) return 'bg-slate-800/30 text-slate-600 border-slate-800 cursor-not-allowed';
        return 'bg-slate-800 text-slate-300 border-slate-700 hover:border-sky-500 hover:text-white';
    };
    ...
    return (
        <div className="space-y-6">
            <div className="rounded-3xl border border-slate-800 bg-slate-900 p-6 space-y-4">
                ...
                <div>
                    <label className="text-sm text-slate-400">Seats</label>
                    <input value={seats} onChange={(e) => { setSeats(Math.max(1, Number(e.target.value))); setSelectedSeatNumbers([]); }} type="number" min="1" className="mt-1 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white" />
                </div>
            ...
            {fareData && !calculatingFare && (
                <div className="rounded-3xl border border-emerald-500/30 bg-emerald-500/5 p-5 space-y-3">
                    <p className="text-xs uppercase tracking-[0.3em] text-emerald-400 font-semibold">Shuttle fare estimate</p>
                    <div className="flex items-center justify-between"><span className="text-slate-400 text-sm">Total ({fareData.seats || seats} seats)</span><span className="text-2xl font-bold text-emerald-400">{formatCurrency(fareData.estimatedTotal || fareData.price)}</span></div>
                    ...
                    {fareData.type === 'fixed' && (
                        <>
                            <div className="flex items-center justify-between text-sm"><span className="text-slate-400">Est. Distance</span><span className="text-white">{fareData.distanceKm ? `${fareData.distanceKm} km` : 'Unknown'}</span></div>
                            {fareData.durationMinutes && <div className="flex items-center justify-between text-sm"><span className="text-slate-400">Est. Duration</span><span className="text-white">{fareData.durationMinutes} min</span></div>}
                            {fareData.departureTimes?.length > 0 && <div className="flex items-center justify-between text-sm"><span className="text-slate-400">Departures</span><span className="text-white">{fareData.departureTimes.join(', ')}</span></div>}
                        </>
                    )}
                    {fareData.seats > 1 && <div className="flex items-center justify-between text-sm"><span className="text-slate-400">Per seat</span><span className="text-white">{formatCurrency((fareData.estimatedTotal || fareData.price) / fareData.seats)}</span></div>}
            ...
            {loading ? (
                <div className="text-slate-400 py-12 text-center">Loading shuttles...</div>
            ) : vehicles.length === 0 ? (
                <div className="rounded-3xl border border-slate-800 bg-slate-900 p-10 text-center"><div className="text-5xl mb-4">🚌</div><h3 className="text-xl font-semibold text-white">No shuttles available</h3></div>
            ) : (
                <div className="space-y-6">
                    {vehicles.map((v) => (
                        <div key={v._id} className="rounded-3xl border border-slate-800 bg-slate-900 p-6 hover:border-slate-700 transition">
                            ...
                            <p className="text-xs text-slate-500 mt-1">{v.availableSeats}/{v.totalSeats} seats available</p>

                            {v.availableSeats > 0 && (
                                <div className="mt-4">
                                    <p className="text-xs text-slate-400 mb-2">Select seats ({selectedSeatNumbers.length}/{seats})</p>
            ...
            {showPayment && selectedVehicle && (
                <PaymentModal amount={paymentAmount} rideData={{ vehicleId: selectedVehicle._id, pickup: { address: pickup, coordinates: pickupCoords || undefined }, dropoff: { address: dropoff, coordinates: dropoffCoords || undefined }, rideType, scheduledTime, customerPhone: phone, fare: fareData, seats, seatNumbers: selectedSeatNumbers }} onSuccess={handlePaymentSuccess} onCancel={() => setShowPayment(false)} />
            )}
        </div>
    );
};

export default ShuttleTransportTab;